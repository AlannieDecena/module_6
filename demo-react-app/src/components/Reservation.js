...
function Reservation() {
    ...
    const [eventDate, setEvenDate] = React.useState('')

    const handleSubmit = (e) => {
        e.preventDefault();

        alert(`${name} is ${isGoing ? '' : 'not '}attending with ${numGuests} guests`)
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="Reservation componentBox">
            ...
            <label>
                Event Date:
                <input name="eventDate" type="date" value={eventDate} onChange={(e) => setEvenDate(e.target.value)} />
            </label>
            <br/>              
